...
const { connect, connection, model, Schema, Types } = mongoose_1.default;
...
const generatePhoneNumber = () => {
    const areaCode = (0, crypto_1.randomInt)(200, 1000); // 200-999 inclusive
    const exchange = (0, crypto_1.randomInt)(200, 1000); // 200-999 inclusive
    const number = (0, crypto_1.randomInt)(0, 10000); // 0-9999 inclusive
    return `${areaCode}-${exchange}-${number.toString().padStart(4, '0')}`;
};
const generateCallSid = () => {
    return `CA${(0, crypto_1.randomUUID)().replace(/-/g, '').substring(0, 32)}`;
};
...
const generateRandomChunks = (transcriptId) => {
    const chunks = [];
...
async function seedData() {
    try {
        ...
        // Create test user
        const hashedPassword = await bcryptjs_1.default.hash('Admin123!', SALT_ROUNDS);
...
//# sourceMappingURL=seed-inbox-data.js.map